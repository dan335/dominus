Template.capitalIncomeGraph.onRendered(function() {
  this.autorun(function() {
    var find = {};
    var options = {sort: {created_at: 1}, fields: {capitalInc:1, created_at:1}};
    var data = Dailystats.find(find, options).map(function(stat) {
      ...
      _s.market.types.forEach(function(type) {
        if (stat.capitalInc && stat.capitalInc[type]) {
    ...
    if (data && data.length) {
      var options = {
        ...
        full_width: true,
        height: 300,
        target: '#capitalIncomeGraph', // the html element that the graphic is inserted in
        x_accessor: 'created_at',  // the key that accesses the x value
        legend_target: '#capitalIncomeGraphLegend',
